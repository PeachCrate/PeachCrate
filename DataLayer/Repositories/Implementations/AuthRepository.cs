...
public class AuthRepository : IAuthRepository
{
    ...
    public async Task<User> RegisterUserAsync(RegisterDTO registerDto)
    {
        ...
        User user = new()
            { Login = registerDto.Login, 
                Email = registerDto.Email, PasswordHash = hash, PasswordSalt = salt, RegistrationDate = DateTime.UtcNow };
    ...
    public async Task<(JwtToken, JwtToken)> LoginAsync(LoginProp prop)
    {
        ...
        bool isPasswordRight = await _passwordHasher.VerifyPasswordHashAsync(prop.Password, userForLogin.PasswordHash, userForLogin.PasswordSalt);
        if (!isPasswordRight)
            throw new Exception("Bad credentials");
